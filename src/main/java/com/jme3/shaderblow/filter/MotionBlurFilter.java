...
public class MotionBlurFilter extends Filter {
    ...
    private final Matrix4f currProjectionMatrix = new Matrix4f();
    private final Matrix4f currToPrevMatrix = new Matrix4f();
    ...
    protected void initFilter(AssetManager manager, RenderManager renderManager, ViewPort vp, int w, int h) {
        material = new Material(manager, "MatDefs/Filters/MotionBlur/MotionBlur.j3md");
        viewPort = vp;
    ...
    private void setCurrToPrevMatrix() {
        currToPrevMatrix.set(currProjectionMatrix); // "previous projection matrix"
        currProjectionMatrix.set(viewPort.getCamera().getViewProjectionMatrix()); // "current projection matrix"
        currToPrevMatrix.multLocal(currProjectionMatrix.invert(tempMat)); // "prev proj mat" * "curr proj mat INVERTED"
        material.setMatrix4("CurrentToPreviousMat", currToPrevMatrix);
    }
}
