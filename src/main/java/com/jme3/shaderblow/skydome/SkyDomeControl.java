package com.jme3.shaderblow.skydome;
...
public class SkyDomeControl implements Control {
    ...
    public SkyDomeControl(AssetManager assetManager, Camera cam, String model, String nightSkyMap, String moonMap, String cloudsMap, String fogAlphaMap) {
        ...
        mat_Sky = new Material(assetManager, "MatDefs/SkyDome/SkyDome.j3md");
            ...
            if (cycleN2D) {
                if (dayAlpha < 1.0f) {
                    ...
                    if (fog != null && controlFog) {
                        viewPort.setBackgroundColor(new ColorRGBA().interpolateLocal(fogNightColor, fogColor, dayAlpha));
                        fog.setFogColor(new ColorRGBA().interpolateLocal(fogNightColor, fogColor, dayAlpha));
                    }
                    if (controlSun)
                        sun.setColor(new ColorRGBA().interpolateLocal(sunNightLight, sunDayLight, dayAlpha));
                } else {
                    dayAlpha = 1.0f;
                    mat_Sky.setFloat("Alpha", dayAlpha);
                    if (fog != null && controlFog) {
